...
export default function Header({ currentRole, onChangeRole, systemAlert, onTriggerAlertReset }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white border-b border-slate-200 shadow-xs" id="app-header">
      {/* Upper Brand Bar */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <div className="p-2.5 bg-gradient-to-tr from-blue-600 to-rose-500 rounded-xl text-white shadow-sm flex items-center justify-center animate-pulse">
        ...
        <div className="flex items-center space-x-4">
          {systemAlert ? (
            <button
              ...
              className="flex items-center space-x-2 px-3.5 py-1.5 bg-rose-50 border border-rose-200 text-rose-700 hover:bg-rose-100 rounded-full text-xs font-medium animate-bounce transition-all shadow-xs"
              id="emergency-clear-btn"
            >
              <ShieldAlert className="h-4 w-4 text-rose-600" />
              <span className="font-semibold">EMERGENCY ALARM ACTIVE - TAP TO DISMISS</span>
            </button>
          ) : (
            <div className="flex items-center space-x-2 px-3.5 py-1.5 bg-slate-50 border border-slate-200 rounded-full text-xs text-slate-600 font-medium">
              ...
              {roles.map((role) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center space-x-2 px-3.5 py-2 rounded-lg text-xs font-semibold tracking-wide transition-all border cursor-pointer ${
                      isActive
